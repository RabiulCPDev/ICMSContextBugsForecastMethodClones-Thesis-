...
public class InvestigatingBugproneness extends javax.swing.JFrame {
    ...
    public void investigation11 (int clonetype)
    {
        ...
        for (int i =1;i<cp.revisionCount;i++)
        {
            ...
            if (bugfixcommits.contains (" " + (i+1) + " "))
            {
                ...
                if (commits[count].changedclones.trim().length() == 0) {l = 0;}
                ...
                for (int j=0;j<l;j++)
                {
                    ...
                    for (gc = 0;gc<gcount;gc++)
                    {
                        if (groups[gc].commit == i && groups[gc].clonegroup.contains(" "+aclone+" ")) 
                        {
                            ...
                            if (spcpclones.contains (" "+aclonegcid+" ")) { groups[gc].affectedSPCPclones += " " + aclonegcid + " "; }
                            break;
                        }
                    }
                    if (gc == gcount) //the clone was not included in any group.
                    {
                        ...
                        if (spcpclones.contains (" "+aclonegcid+" ")) {groups[gcount].affectedSPCPclones += " " + aclonegcid + " ";}
                        gcount++;
                    }
                }
                count++;
    ...
    public String getCommitter (int commitnum)
    {
        ...
        {
            if (commits[i] == commitnum) { return committers[i]; }
        }
        return "";
    }
    ...
    public void analyzeBugpronenessOfClones (int clonetype)
    {
        ...
        for (int r = 1;r <=cp.revisionCount-1;r++)
        {
            ...
            {
                if (clones[i].changecount == null) {continue;}
                if (Integer.parseInt(clones[i].changecount)==0) {continue;}
    ...
    public void investigation4 (int clonetype)
    {
        ...
        for (int i =0;lps[i] != null;i++)
        {
            ...
            if (str != null && !str.equals ("null")) { temp1 = Integer.parseInt (str); }
            ...
            if (str != null && !str.equals ("null")) { temp2 = Integer.parseInt (str); }
        ...
        if (bugfixclones.trim().length () > 0) { bugfixclonecount = bugfixclones.trim().split("[ ]+").length; }
        
        int lpclonecount = 0;
        if (lpclones.trim().length () > 0) { lpclonecount = lpclones.trim().split("[ ]+").length; }
        ...
        for (int r =1;r<cp.revisionCount;r++)
        {
            ...
            for (int i =0;lps[i] != null;i++)
            {
                if (lps[i].convergedin.equals (r+""))
                {
                    ...
                    if (changed1 > 0)
                    {
                        if (!clones.contains (" "+gcid1+" ")){ clones += ", " + gcid1+" "; }
                        if (!lpclones.contains (" "+gcid1+" ")){ lpclones += " " + gcid1 + " "; }
                        if (bugfixcommits.contains (" "+r+" "))
                        {
                            if (!lpbugfixclones.contains (" "+gcid1+" ")){ lpbugfixclones += " " + gcid1 + " "; }
                        }
                    }
                    ...
                    if (changed2 > 0)
                    {
                        if (!clones.contains (" "+gcid2+" ")){ clones += ", " + gcid2 + " "; }
                        if (!lpclones.contains (" "+gcid2+" ")){ lpclones += " " + gcid2 + " "; }
                        if (bugfixcommits.contains (" "+r+" "))
                        {
                            if (!lpbugfixclones.contains (" "+gcid2+" ")){ lpbugfixclones += " " + gcid2 + " "; }
                        }
                    }
                }
            }
                ...
                for (int j = 0;j<l-1;j++)
                {
                    ...
                    for (int k =j+1;k<l;k++)
                    {
                        ...
                        if (result == 1)
                        {
                            ...
                            if (mclone1 == 1)
                            {
                                if (!spcomethodclones.contains (" " + gcid1 + " ")) { spcomethodclones += " " + gcid1 + " "; }
                            }
                            if (mclone2 == 1)
                            {
                                if (!spcomethodclones.contains (" " + gcid2 + " ")) { spcomethodclones += " " + gcid2 + " "; }
                            }
                        }
                    }
                }
    ...
    public void investigation8 (int clonetype)
    {
        ...
        int bfcount = 0;
        if (bugfixclones.trim().length() > 0) { bfcount =  bugfixclones.trim().split("[ ]+").length;}
        
        int spcpcount = 0;
        if (spcpclones.trim().length() > 0) { spcpcount =  spcpclones.trim().split("[ ]+").length;}
        
        int bfspcpcount = 0;
        if (bugfixspcpclones.trim().length() > 0) { bfspcpcount =  bugfixspcpclones.trim().split("[ ]+").length;}
        
        int bfspcpcountlr = 0;
        if (bugfixspcpcloneslastrevision.trim().length() > 0) { bfspcpcountlr =  bugfixspcpcloneslastrevision.trim().split("[ ]+").length;}
        ...
        for (int i =1;i<cp.revisionCount;i++)
        {
            ...
            if (bugfixcommits.contains (" " + (i+1) + " "))
            {
                ...
                for (int j=0;clones[j] != null;j++)
                {
                    ...
                    {
                        commits[count].changedclones += " " + clones[j].globalcloneid + " ";
                        if (isMethodClone(Integer.parseInt(clones[j].globalcloneid), clonetype, i) == 1) { commits[count].changedmethodclones += " " + clones[j].globalcloneid + " "; }
    ...
    public void investigation10 (int clonetype) throws Exception
    {
        ...
        for (int i =0;groups[i] != null;i++)
        {
            ...
            if (aclones.trim().split("[ ]+").length < 2) {continue;}
            groupno++;
